...
public final class Sprite extends DrawingArea {
	...
	public Sprite(String img, int width, int height) {
		try {
			Image image = Toolkit.getDefaultToolkit().createImage(FileOperations.ReadFile(location + img + ".png"));
			myWidth = width;
			myHeight = height;
			maxWidth = myWidth;
			maxHeight = myHeight;
	...
	public Sprite(NamedArchive archive, String s, int i) {
		ByteBuffer stream = new ByteBuffer(archive.getDataForName(s + ".dat"));
		ByteBuffer stream_1 = new ByteBuffer(archive.getDataForName("index.dat"));
	...
	public void drawTransSprite(int i, int j) {
		int k = 128;//was parameter 225
		i += anInt1442;
		j += anInt1443;
		int i1 = i + j * DrawingArea.width;
		int j1 = 0;
	...
	public void manipulateSprite(int x, int reduceX, int y, int reduceY) {
		x += anInt1442;
		y += anInt1443;
