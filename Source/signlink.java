...
public final class signlink implements Runnable {
    ...
    public void run() {
        ...
        uid = getuid(dir);
        try {
            File file = new File(dir + "main_file_cache.dat");
            if(file.exists() && file.length() > 2147000000)
                file.delete();
            cache_dat = new RandomAccessFile(dir + "main_file_cache.dat", "rw");
            for(int j = 0; j < 5; j++)
                cache_idx[j] = new RandomAccessFile(dir + "main_file_cache.idx" + j, "rw");
        ...
        for(int i = threadliveid; threadliveid == i;) {
            if(socketreq != 0) {
                ...
                if(savebuf != null)
                    try {
                        FileOutputStream fileoutputstream = new FileOutputStream(dir + savereq);
                        fileoutputstream.write(savebuf, 0, savelen);
                        fileoutputstream.close();
                    } catch(Exception _ex) {
					}
                if(waveplay) {
                    String wave = dir + savereq;
                    waveplay = false;
                }
                if(midiplay) {
                    midi = dir + savereq;
